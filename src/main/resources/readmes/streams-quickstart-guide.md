...
## Реактивные твиты (см. файл `ru.reactive_tweets.TwitterStreamQuickstartDocSpec` в папке с тестами)
...
определенную информацию, например, найти все твиттер упоминания пользователей, которые чирикают о `#akka`.

Чтобы подготовить нашу среду, создав `ActorSystem` и `ActorMaterializer`, которые будут отвечать за материализацию и 
